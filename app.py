...
SCRAPER_API_KEY = os.environ.get('SCRAPER_API_KEY', '')  # من scraperapi.com — مجاني 1000 req/شهر
...
# ── TikTok via tikwm.com (مجاني بدون حد) ────────────────────────────────────
def fetch_tiktok_tikwm(username, cursor=0, count=20):
    """جلب فيديوهات البروفايل عبر tikwm.com"""
    clean = username.lstrip('@')
    # نجرب أكتر من endpoint
    endpoints = [
        ('POST', 'https://www.tikwm.com/api/user/posts',
         {'unique_id': clean, 'count': count, 'cursor': cursor, 'hd': 1}),
        ('GET',  f'https://tikwm.com/api/user/posts?unique_id={clean}&count={count}&cursor={cursor}&hd=1',
         None),
        ('POST', 'https://api.tikwm.com/api/user/posts',
         {'unique_id': clean, 'count': count, 'cursor': cursor}),
    ]
    headers = {
    ...
    for method, url, data in endpoints:
        try:
            ...
            log.warning(f"tikwm endpoint {url} failed: {e}")

    # لو كل الـ endpoints فشلت، نجرب عبر ScraperAPI proxy
    if SCRAPER_API_KEY:
        try:
            proxy_url = f"http://api.scraperapi.com/?api_key={SCRAPER_API_KEY}&url=https://www.tikwm.com/api/user/posts&render=false"
            r = requests.post(
                f"http://api.scraperapi.com/",
                params={'api_key': SCRAPER_API_KEY, 'url': 'https://www.tikwm.com/api/user/posts'},
                data={'unique_id': clean, 'count': count, 'cursor': cursor, 'hd': 1},
                timeout=30
            )
            if r.status_code == 200:
                j = r.json()
                if j.get('code') == 0:
                    log.info(f"tikwm via ScraperAPI success!")
                    return j
        except Exception as e:
            log.error(f"ScraperAPI proxy failed: {e}")

    log.error(f"All tikwm endpoints failed for {username}")
    return None

def fetch_tiktok(username, fetch_all=False):
    clean, results, cursor, page = username.lstrip('@'), [], 0, 0
    while True:
        # نجرب tikwm.com أولاً (مجاني بدون حد)
        data = fetch_tiktok_tikwm(clean, cursor=cursor)

        # لو فشل tikwm، نجرب RapidAPI
        if not data or data.get('code') != 0:
            log.warning(f"tikwm failed, trying RapidAPI...")
            data = None
            tried = 0
        ...
        if not data:
            log.error(f"All APIs failed for {username}")
            break

        # tikwm response format
        if 'data' in data and isinstance(data['data'], dict) and 'videos' in data['data']:
            d = data['data']
            raw_items = d.get('videos', [])
            items = []
            ...
            has_more = d.get('hasMore') or False
            next_cursor = d.get('cursor') or 0
        else:
            # RapidAPI format
            d = data.get('data') or data
            raw_items = d.get('videos') or d.get('aweme_list') or d.get('itemList') or (d if isinstance(d, list) else [])
            items = []
            for item in raw_items:
                vid_id = str(item.get('video_id') or item.get('aweme_id') or item.get('id') or '')
                dur = int(item.get('duration') or item.get('video', {}).get('duration', 0) or 0)
            ...
            next_cursor = d.get('cursor') or d.get('nextCursor') or 0

        if not items: break
        for item in items:
            if not item['video_id'] or item['duration'] > MAX_SECS: continue
            results.append(item)

        page += 1
        if not fetch_all: break
        if not has_more or not next_cursor or page >= 50: break
        cursor = next_cursor
        time.sleep(0.8)
    log.info(f"[{username}] fetched {len(results)} videos in {page} pages")
    return results
