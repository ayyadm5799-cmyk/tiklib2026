...
def fetch_tiktok(username, count=30, fetch_all=False):
    """
    fetch_all=True  → يجيب كل فيديوهات البروفايل بـ pagination
    fetch_all=False → يجيب آخر `count` فيديو بس (للتحديث الدوري)
    """
    clean   = username.lstrip('@')
    results = []
    cursor  = 0
    page    = 0
    max_pages = 50  # حماية من اللوب اللانهائي (50 × 30 = 1500 فيديو كحد أقصى)
    ...
    while True:
        ...
        # لو مش fetch_all، نوقف بعد أول صفحة
        if not fetch_all:
            break

        # في حالة fetch_all: نكمل لو فيه صفحة تانية
        ...
        time.sleep(0.5)  # delay خفيف بين الصفحات

    log.info(f"[{username}] TikTok fetched {len(results)} videos in {page} pages")
    return results
...
def sync_profile(profile_id, username, url, platform, new_only=False):
    ...
    try:
        ...
        if platform == 'tiktok':
            # أول مرة: جيب كل الفيديوهات | تحديث دوري: آخر 30 بس
