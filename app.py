...
def upload_to_dropbox(video_url, username, video_id, title=''):
    """تحميل الفيديو مباشرة من الرابط ورفعه على Dropbox"""
    if not DROPBOX_TOKEN:
        log.warning("DROPBOX_TOKEN غير موجود")
        return None
    try:
        ...
        # مسار الملف على Dropbox
        safe_title = ''.join(c for c in (title or video_id)[:40] if c.isalnum() or c in ' _-')
        dropbox_path = f"/TikLib/{username.lstrip('@')}/{video_id}.mp4"
    ...
    try:
        ...
        for v in items:
            ...
            try:
                ...
                # رفع تلقائي على Dropbox
                if DROPBOX_TOKEN:
                    _url = v.get('nowm_url') or v.get('play_url','')
                    if _url:
                        threading.Thread(
                            target=upload_to_dropbox,
                            args=(_url, username, v['video_id'], v.get('title','')),
                            daemon=True
                        ).start()
            except Exception as e:
